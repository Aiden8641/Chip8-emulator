...
void Chip8::loadROM(char const *filename) {
  ...
  if (file.is_open()) {
    // get size of file and allocate buffer to hold content
    std::streampos size{file.tellg()};
    char *buffer = new char[size]; // using char cause it represetns exactly 1
                                   // byte so the size of the buffer is the same
                                   // as the size of the file

    // go back to the begining of file and fill buffer
...
void Chip8::OP_3xkk() {
  ...
  uint8_t byte{static_cast<uint8_t>(opcode & 0x00FFu)};
...
void Chip8::OP_Dxyn() {
  uint8_t Vx{static_cast<uint8_t>((opcode & 0x0F00u) >> 8u)};
  uint8_t Vy{static_cast<uint8_t>((opcode & 0x00F0u) >> 4u)};
  uint8_t height{static_cast<uint8_t>(opcode & 0x000Fu)};

  // wraps around screen
  uint8_t xPos{static_cast<uint8_t>(registers[Vx] % VIDEO_WIDTH)};
  uint8_t yPos{static_cast<uint8_t>(registers[Vy] % VIDEO_HEIGHT)};
...
void Chip8::OP_Fx33() {
  ...
  uint8_t ones{static_cast<uint8_t>(decimal % 10)};
  memory[index + 2] = ones;
  decimal /= 10;

  uint8_t tens{static_cast<uint8_t>(decimal % 10)};
  memory[index + 1] = tens;
  decimal /= 10;

  uint8_t hundreds{static_cast<uint8_t>(decimal % 10)};
